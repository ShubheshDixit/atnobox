...
import imageBox from '../assets/Images.png'
...
function Home() {
    ...
    return (
    <div className="w-full h-full flex flex-col justify-start items-center overflow-x-auto mx-8">
        <div className="flex flex-col bg-bg-light justify-center items-center w-full p-6">
            ...
            <h1 className="font-bold text-xl text-primary-light"><FontAwesomeIcon icon={faBox} className="mr-2" size="1x"/> AtnoBox</h1>
                <div className="flex-row justify-center items-center hidden sm:flex">
                    <Link to="/" className="p-4 h-12 flex justify-center items-center bg-primary-light mx-2 rounded-md bg-opacity-100 text-black"><h2 className="font-bold">Links</h2></Link>
                    <Link to="/files" className="p-4 flex justify-center items-center h-12 bg-primary-light mx-2 rounded-md bg-opacity-10"><h2 className="font-bold">Files</h2></Link>
                </div>
                {user != null ?
       ...
       {user == null && !loading? 
        ...
        {user !=null ? 
            <div className="w-full max-w-screen-md flex flex-col justify-center items-center py-8">
                <img src={imageBox} className="h-64 w-64 object-contain"/>
                <h1 className="font-bold text-4xl my-2">AtnoLinks</h1>
                <p className="text-xl text-center">Save and share your links from everywhere.</p>
                <div className="flex flex-row  w-full max-w-screen-md p-4 justify-center items-center">
                    <PrimaryButton onClick={()=>{ history.push('add-project') }}>Creat a project</PrimaryButton>
                    <SecondaryButton onClick={()=>{ history.push('add-link') }}>Add a new link</SecondaryButton>
                </div>
         ...
         {user != null ?
            <div className="w-full max-w-screen-md flex flex-col justify-center items-center p-4">
                <div className="w-full border-primary-light border-b-2 border-dashed border-opacity-25 my-4 flex flex-row border-collapse justify-center items-center">
                    <button className={`py-4 mx-4 font-bold ${index === 0 ? 'border-b-4 border-primary-light' : ''}`}  onClick={()=>{setIndex(0)}}>Projects</button>
                    <button className={`py-4 mx-4 font-bold ${index === 1 ? 'border-b-4 border-primary-light' : ''}`} onClick={()=>{setIndex(1)}}>Link</button>
                </div>
                {index == 0 ?
                    <ProjectView></ProjectView> : <LinksView></LinksView>
                }
            </div>: null
        }
